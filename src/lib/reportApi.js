...
export const reportApi = {
  ...
  protected: {
    ...
    getMyReports: async (params) => {
      const response = await protectedClient.get('/reports/my-reports', { params });
      return response.data;
    },
    ...
    getVerifiedReports: async (params) => {
      const response = await protectedClient.get('/reports/verified', { params });
      return response.data;
    },

    // Verify a report (admin/authorized users only)
    verifyReport: async (reportId, verificationData) => {
      const response = await protectedClient.post(`/reports/${reportId}/verify`, verificationData);
      return response.data;
    },

    // Dismiss a report (admin/authorized users only)
    dismissReport: async (reportId, dismissData) => {
      const response = await protectedClient.post(`/reports/${reportId}/dismiss`, dismissData);
      return response.data;
    }
  }
};